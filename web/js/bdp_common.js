...
var BEHEMOTH_COMMON = __BDP_C;
/**
 * @author Ewen
 * 全局参数设定和初始化
 */

__BDP_C.getQueryString = function getQueryString($name, $url) {
 ...
 * @param str：要获得长度的字符串
 */
__BDP_C.strLen = function strLen(str) {
    var len = 0;
 ...
 * @param len: 需要截取的长度
 *
 * console.log(strCut("JS~字符串长度判断，超出进行自动截取（支持中文）", 30));
 */
__BDP_C.strCut = function strCut(str, len) {
    var str_length = 0;
    var str_len = 0;
    str_cut = new String();
    str_len = str.length;
...
__BDP_C.fmtNum = function fmtNum(s, n) {
    n = n >= 0 && n <= 20 ? n : 2;
    ...
    var x = t.split("").reverse().join("");
    return n > 0 ? x + "." + r : x;
};


__BDP_C.return2Br = function (str) {
    return str.replace(/\r?\n/g, "<br />");
};

//去掉html标签
__BDP_C.removeHtmlTab = function (tab) {
    return tab.replace(/<[^<>]+?>/g, '');//删除所有HTML标签
};

//普通字符转换成转意符
__BDP_C.html2Escape = function (sHtml) {
    return sHtml.replace(/[<>&"]/g, function (c) {return {'<': '&lt;', '>': '&gt;', '&': '&amp;', '"': '&quot;'}[c];});
};

//转意符换成普通字符
__BDP_C.escape2Html = function (str) {
 ...
 */
__BDP_C.MD5 = function MD5(string) {
