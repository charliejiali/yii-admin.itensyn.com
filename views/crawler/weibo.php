...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        ...
        $('#new').on('click',function(){
            window.location.href='/crawler/weibo-edit?act=add'
        });
        $('#table-head').on('click','a[id^="edit_"]',function(){
            window.location.href='/crawler/weibo-edit?act=edit&name='+$(this).attr('id').split('_')[1];
        });
        $('#search').on('click',function(){
            window.location.href='/crawler/weibo-list?q='+$.trim($('#q').val())+'&s='+$('#crawler_status option:selected').val()
        });
        $('#q').on('keypress',function(e){
            if(e.keyCode==13){
                window.location.href='/crawler/weibo-list?q='+$.trim($(this).val())+'&s='+$('#crawler_status option:selected').val()
            }
        });
    });
</script>
